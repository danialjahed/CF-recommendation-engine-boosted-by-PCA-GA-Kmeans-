...
def Dot(iterator,k,l,R,u,start,end,return_value):
    ...
    for a in range(start,end):
        ...
        if n != 0:
            q = (R+(m/n))
            result.append([a,u[a],q,abs((q-u[a]))])
        else:
            result.append([a,u[a],np.nan,np.nan])
...
#   load test dataset
X_NAN_ALL = pd.read_csv("X_original_complete_NAN.csv")
...
# p_sorted.to_csv("sorted_predict_rates.csv")
...
while n != 30:
    ...
    if np.isnan(film_info[1]):
        recom_film_id.append(film_info[0]+1)
        n +=1
    cntr += 1
